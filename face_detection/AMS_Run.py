...
def manually_fill():
    ...
    def fill_attendance():
        ...
        try:
            global cursor
            connection = pymysql.connect("localhost", "root", "", "manually_fill_attendance");
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                csv_name='C:/SGP 2/face_detection/Attendance/Manually Attendance/'+DB_table_name+'.csv'
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"C:\SGP 2\face_detection\Attendance\Manually Attendance\-------Check atttendance-------"')
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                try:
                    global cursor
                    connection = pymysql.connect("localhost","root","", "Face_reco_fill")
                    cursor = connection.cursor();
                except Exception as e:
                    print(e);
                ...
                root.configure(background='snow')
                cs = 'C:/SGP 2/face_detection/' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"C:\SGP 2\face_detection\Attendance\-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'FAMS' :
            if password == 'FAMS':
                ...
                root.configure(background='snow')
                cs = 'C:/SGP 2/face_detection/StudentDetails/StudentDetails.csv'
                msft = pd.read_csv(cs)

                mylist = Listbox(root, yscrollcommand=scrollbar.set,bg="Green",fg="White",width=1080)
                with open(cs, 'r') as f:
                    reader = csv.reader(f)
